...
if ($method === 'GET') {
    ...
    // FIX #1 (critical): test_auth.html submits a regular HTML form
    // (Content-Type: application/x-www-form-urlencoded), but the original code
    // only read JSON. Now we accept BOTH:
    //   - JSON body (used by script.js fetch calls)
    //   - $_POST fields (used by test_auth.html plain form submissions)
    $raw   = file_get_contents('php://input');
    $input = json_decode($raw, true);
    if (!is_array($input) || empty($input)) {
        // Fallback: plain form submission from test_auth.html
        $input = $_POST;
    }
...
// ─────────────────────────────────────────────
// FIX #6 (medium): getDB() was called throughout this file but never defined.
// Every authenticated action (register, login, delete) threw a fatal PHP error.
// This function retrieves the $pdo connection created by db.php.
// ─────────────────────────────────────────────
function getDB(): PDO {
    global $pdo;
    return $pdo;
...
function handleLogin(array $data): void {
    ...
    $pdo  = getDB();
    $s    = $pdo->prepare('SELECT id, name, email, password FROM users WHERE email = ?');
    $s->execute([$email]);
    $user = $s->fetch();
